class Level {
    ...
    for (int y=0; y < numOfTiles.y; y++) {
      for (int x=0; x < numOfTiles.x; x++) {
        ...
        if (character == 'k') {
          ...
          extObjs.soldiers.add(new SoldierEnemy(new PVector(x*tileHeight, y*tileWidth), new PVector(1, 0), new PVector(0, 0), 0.3));  
        }
  ...
  void display(Mario theMario) {
    for (int y=0; y < numOfTiles.y; y++) {
      for (int x=0; x < numOfTiles.x; x++) {
        thetile[x][y].display(grav, theMario, extObjs.koopas, extObjs.soldiers);
